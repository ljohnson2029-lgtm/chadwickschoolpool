...
-- Recreate with connected parent info
...
RETURNS TABLE(
  ...
  parent_email text,
  connected_parent_first_name text,
  connected_parent_last_name text
)
LANGUAGE sql
SECURITY DEFINER
...
AS $$
  ...
  SELECT
    ...
    cp.first_name AS connected_parent_first_name,
    cp.last_name AS connected_parent_last_name
  FROM public.account_links al
  JOIN public.profiles pp ON pp.id = al.parent_id
  JOIN public.rides r ON r.user_id = al.parent_id
  LEFT JOIN public.ride_conversations rc 
    ON rc.ride_id = r.id AND rc.status = 'accepted'
  LEFT JOIN public.profiles cp 
    ON cp.id = CASE 
      WHEN rc.sender_id = al.parent_id THEN rc.recipient_id 
      ELSE rc.sender_id 
    END
  ...
  SELECT
    ...
    pp.last_name  AS parent_last_name,
    public.get_user_email(al.parent_id) AS parent_email,
    op.first_name AS connected_parent_first_name,
    op.last_name AS connected_parent_last_name
  FROM public.account_links al
  JOIN public.profiles pp ON pp.id = al.parent_id
  ...
  SELECT
    ...
    pp.last_name  AS parent_last_name,
    public.get_user_email(al.parent_id) AS parent_email,
    opp.first_name AS connected_parent_first_name,
    opp.last_name AS connected_parent_last_name
  FROM public.account_links al
  JOIN public.profiles pp ON pp.id = al.parent_id
  ...
  LEFT JOIN public.profiles opp 
    ON opp.id = CASE 
      WHEN pr.sender_id = al.parent_id THEN pr.recipient_id 
      ELSE pr.sender_id 
    END
  WHERE auth.uid() = student_user_id
    AND al.student_id = student_user_id
